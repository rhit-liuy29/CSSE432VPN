...
Go back to the destination server created in step 3, run server.py that was imported in step 4. server.py requires both IP and port number to run, make sure you put your CA Server IP as IP and the same port number of the running CA Server in step 1. Then the python script should take care of the rest. You will receive a signed crt file under /easyrsa/pki/issued/ directory if all goes well. Also, logs will also be generated to help you identify what is wrong. 
